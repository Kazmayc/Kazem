...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@https://t.me/o10_0o"))
...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@o10_0o"))
    except BaseException:
        pass  
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@K_4_ll"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : أناتوليوس"
    )
...
async def _(event):
    ...
    2130688215 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"2130688215 : `{event.sender_id}`")
    else:
